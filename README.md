...
- 배포: Docker Compose (`.env`는 dev/prod 공용 단일 파일, dev/prod 구분은 `compose.sh`가 로드하는 compose 파일로만 결정)

## 환경 변수

`.env` 파일 하나만 사용합니다 (dev/prod 겸용). `APP_MODE`는 `.env`에 두지 않고 `compose.dev.yaml`/`compose.prod.yaml`에 하드코딩되어 있으므로 신경 쓸 필요가 없습니다.
