...
- 배포: Docker Compose + `compose.sh`. **`compose.yaml` 단일 파일**(2026-07-07 결정, dev/prod 오버라이드 파일 없음 — 아래 9~10번 참고), **`.env`도 dev/prod 공용 단일 파일**(2026-07-06 결정).
...
- compose 파일 정책 → **`compose.yaml` 단일 파일, dev/prod 오버라이드 파일 없음** (2026-07-07 결정, 아래 10번 참고). `compose.dev.yaml`/`compose.prod.yaml`, `compose.sh`의 `--dev` 플래그는 전부 제거됨 — 되살리지 말 것.
...
## 8.2 알려진 함정: 다중 워커와 스키마 생성 경합

- 백엔드(`backend/Dockerfile`)는 `uvicorn --workers 2`로 뜬다. 각 워커가 FastAPI `lifespan`을 독립적으로 실행하므로, `Base.metadata.create_all()`을 그냥 호출하면 **빈 DB에 처음 뜰 때 두 워커가 동시에 같은 테이블을 CREATE하려다 `pg_type_typname_nsp_index` 유니크 제약 위반으로 백엔드가 죽는다** (2026-07-06 실제 운영 서버에서 발생/재현/수정함).
...
- 검증 방법: 로컬에서 `docker build -t nugadesk-backend ./backend`로 이미지를 만들고, 빈 Postgres 컨테이너에 붙여 재현할 수 있다 (venv로는 재현 안 됨 — 반드시 Docker 이미지로 테스트).

## 8.3 알려진 함정: self-hosted 러너 체크아웃 권한이 프론트엔드 정적 파일 403으로 이어짐

- self-hosted 러너(NAS)에서 `git reset --hard origin/main`으로 체크아웃되는 파일의 권한은 러너 프로세스의 umask에 좌우된다. `frontend/Dockerfile`이 `COPY --from=build /app/dist /usr/share/nginx/html`로 파일을 복사할 때 원본 권한 비트를 그대로 유지하므로, 러너의 umask가 제한적이면(예: 그룹/기타 읽기 권한 없음) 이미지 안의 정적 파일이 root만 읽을 수 있는 권한으로 들어갈 수 있다. nginx는 non-root(`nginx`) 워커로 뜨기 때문에 그 파일을 읽지 못해 **해당 파일만 개별적으로 `403 Forbidden`을 반환**한다 (다른 파일은 예전 체크아웃 권한 그대로라 멀쩡해서 특정 파일만 깨진 것처럼 보임 — 2026-07-06 `frontend/public/appicon.png` 등 아이콘 파일 교체 후 실제 운영에서 발생/재현/수정함).
- 해결: `COPY --from=build` 직후 `RUN chmod -R a+rX /usr/share/nginx/html`로 정적 파일 전체를 강제로 세계-읽기 가능하게 정규화한다. 호스트 체크아웃 권한에 의존하지 않도록 이 단계를 반드시 유지할 것 — 정적 파일 복사/배포 방식을 바꾸는 경우에도 동일한 권한 정규화가 재발하지 않는지 확인해야 한다.
...
**`compose.yaml` 단일 파일만 쓴다 (2026-07-07 결정). `compose.dev.yaml`/`compose.prod.yaml` 같은 오버라이드 파일과 `compose.sh`의 `--dev` 플래그는 전부 제거됐고, 다시 만들지 않는다.** dev/prod 컨테이너 빌드를 분리하던 이유(핫리로드용 볼륨 마운트, 다른 포트)가 실익보다 관리 비용이 커서 걷어냈다 — 로컬에서 코드를 바로바로 확인할 때는 Docker가 아니라 `frontend`에서 `npm run dev`, `backend`에서 `uvicorn app.main:app --reload`를 직접 띄운다 (Docker는 실제 배포 형태 그대로를 검증/운영하는 용도로만 쓴다).
...
- `APP_MODE`는 `.env`에서 설정 가능하며 기본값은 `prod`다 (`compose.yaml`의 `${APP_MODE:-prod}`). 별도 dev 컨테이너 모드는 없으므로, 헤더 뱃지에서 dev 표시를 보고 싶을 때만 `.env`에 `APP_MODE=dev`를 넣는 용도로 쓴다 — 인프라(빌드 방식/포트)는 바뀌지 않는다.
- 노출 포트는 `compose.yaml`에 고정: 프론트엔드(nginx) `7090:80`, 백엔드 `8000:8000`. dev/prod에 따라 포트가 달라지던 과거 방식은 없앴다 — 포트를 바꿀 일이 있으면 `compose.yaml` 한 곳만 고치면 된다.
- 백엔드/프론트엔드 Dockerfile 모두 **멀티스테이지 dev 타깃을 제거**했다 (`backend/Dockerfile`은 단일 스테이지, `frontend/Dockerfile`은 `build`→최종 nginx 스테이지만 존재). Docker 기반 핫리로드 개발 환경을 다시 원한다면 이 결정을 먼저 사용자에게 재확인할 것 — 한 번 걷어낸 구조이기 때문.
